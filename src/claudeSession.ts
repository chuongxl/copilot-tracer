...
export interface ClaudeTurnContext {
  entry: TraceEntry;
  /** tool correlation key → the ToolCall stored on `entry.toolCalls` */
  tools: Map<string, ToolCall>;
  sessionId: string;
  promptId?: string;
  startedAtMs: number;
  model?: string;
  /** true once Stop/SessionEnd closed the turn; kept around for late OTLP enrichment */
  closed: boolean;
}
...
export interface ClaudeUsageDelta {
  ...
  reasoning?: number;
  written?: number;
  /** Pre-computed credits. When omitted, credits are derived from input/output + model. */
  credits?: number;
  model?: string;
}
...
const activeBySession = new Map<string, ClaudeTurnContext>();
/** promptId → turn (open or closed). The join key shared with OTLP's `prompt.id`. */
const turnsByPromptId = new Map<string, ClaudeTurnContext>();
/** sessionId → most recently closed turn, so usage arriving after Stop still lands */
const lastClosedBySession = new Map<string, ClaudeTurnContext>();
/** buffered usage for turns that haven't been created yet, keyed `p:<promptId>`/`s:<sessionId>` */
const pendingUsage = new Map<string, { delta: ClaudeUsageDelta; at: number }>();
/**
 ...
 */
const hookSessions = new Map<string, number>();
...
export function detectClaudeToolType(name: string): ToolCall['type'] {
  const n = name.toLowerCase();
  if (n.startsWith('mcp__') || n.startsWith('mcp_')) return 'mcp';
  if (n === 'task' || n === 'agent' || n.includes('subagent')) return 'agent';
  if (n === 'skill' || n.startsWith('skill__') || n.includes('skill')) return 'skill';
  return 'builtin';
}

function recount(entry: TraceEntry): void {
...
function persist(ctx: ClaudeTurnContext, done = false): void {
  recount(ctx.entry);
  upsertTrace(ctx.entry);
  traceEvents.emit('trace:update', ctx.entry);
  if (done) traceEvents.emit('trace:done', ctx.entry);
}

// ── Pending-usage buffering ───────────────────────────────────────────────────

function pendingKeys(sessionId?: string, promptId?: string): string[] {
  const keys: string[] = [];
  if (promptId) keys.push(`p:${promptId}`);
  if (sessionId) keys.push(`s:${sessionId}`);
  return keys;
}

function bufferUsage(delta: ClaudeUsageDelta, sessionId?: string, promptId?: string): void {
...
function mergeDelta(target: ClaudeUsageDelta, extra: ClaudeUsageDelta): void {
...
function expirePending(): void {
  ...
  for (const [key, value] of pendingUsage) {
    if (value.at < cutoff) pendingUsage.delete(key);
  }
}

function drainPending(ctx: ClaudeTurnContext): void {
  ...
  if (!lastClosedBySession.has(ctx.sessionId)) keys.push(`s:${ctx.sessionId}`);
...
function addUsage(ctx: ClaudeTurnContext, delta: ClaudeUsageDelta): void {
 ...
 */
export function findClaudeTurn(sessionId?: string, promptId?: string): ClaudeTurnContext | undefined {
  if (promptId) {
...
export function isHookTracked(sessionId?: string): boolean {
  return !!sessionId && hookSessions.has(sessionId);
}

function markHookSession(sessionId: string): void {
...
export function startTurn(input: {
  ...
  if (stale && stale !== existing && !stale.closed) closeTurn(stale, { status: 'done' });
...
function closeTurn(ctx: ClaudeTurnContext, opts: { status?: TraceEntry['status']; response?: string; error?: string }): void {
  if (!ctx.closed) {
    ctx.entry.durationMs = ctx.entry.durationMs || Date.now() - ctx.startedAtMs;
    ctx.closed = true;
  }
  if (opts.response) ctx.entry.response = opts.response;
  if (opts.error) ctx.entry.error = opts.error;
  ...
  if (activeBySession.get(ctx.sessionId) === ctx) activeBySession.delete(ctx.sessionId);
...
export function endSession(sessionId: string): void {
  ...
  if (ctx && !ctx.closed) closeTurn(ctx, { status: 'done' });
  activeBySession.delete(sessionId);
  hookSessions.delete(sessionId);
}

export function registerSession(sessionId: string, projectId?: string): void {
  markHookSession(sessionId);
  createSession(sessionId, projectId);
}
 ...
 */
function toolKey(toolUseId: string | undefined, name: string, input: unknown): string {
 ...
 */
function turnForTool(sessionId: string, promptId: string | undefined, projectId?: string): ClaudeTurnContext {
...
export function finishToolCall(input: {
  ...
  call.endedAt = Date.now();
  call.durationMs = call.endedAt - call.startedAt;
  if (input.output !== undefined) call.output = input.output;
...
export function enrichClaudeContent(input: {
  ...
  if (input.prompt && placeholder.test(ctx.entry.prompt)) ctx.entry.prompt = input.prompt;
  if (input.response && !ctx.entry.response) ctx.entry.response = input.response;
  persist(ctx);
  return true;
}

/** Test-only: drop all in-memory state. */
export function resetClaudeTracker(): void {
