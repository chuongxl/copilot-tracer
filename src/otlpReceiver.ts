  ...
  for (const span of spans) {
    ...
    if (spanName === 'claude_code.interaction') {
      ...
      if (isHookTracked(claudeSessionId)) {
        ...
        if (pendingDelta) pendingClaudeLlmDeltas.delete(traceId);
        applyClaudeUsage({
...
function detectToolType(name: string): ToolCall['type'] {
  ...
  if (n === 'apply_patch' || n === 'shell' || n === 'exec' || n === 'local_shell') return 'builtin';
...
const codexInFlight = new Map<string, TraceEntry>();
...
function codexEventId(record: OtlpLogRecord, attrs: OtlpKeyValue[], eventName: string): string {
...
function processCodexLogRecord(
  record: OtlpLogRecord,
  resourceAttrs: OtlpKeyValue[],
  sessionId: string,
  ...
  if (name === 'codex.tool_decision') {
    ...
    if (!entry) return; // no open turn to attach this tool call to
  ...
  if (name === 'codex.turn_cost' || name === 'codex.sse_event') {
    const id = codexEventId(record, attrs, name);
    const entry = codexInFlight.get(id);
    if (!entry) return; // no open turn — usage backfill with nothing to backfill
...
function processCodexLogs(payload: OtlpLogsPayload, defaultSessionId: string, projectId?: string): void {
