...
${prompt.trim()}`;
    ...
    try {
      const raw = execSync(
        `copilot -p ${JSON.stringify(metaPrompt)} --model claude-sonnet-4.6`,
      ...
      if (!jsonMatch) throw new Error('No JSON in response: ' + cleaned.slice(0, 200));
      const result = JSON.parse(jsonMatch[0]) as { optimized?: string; issues?: unknown[]; techniques?: string[] };
  ...
  app.post('/api/work-items/:id/draft', (req, res) => {
    const { overwriteUserEdits } = req.body as { overwriteUserEdits?: unknown };
