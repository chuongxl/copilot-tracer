...
function emptyEvidence(projectPath: string | null, error: string | null): GitEvidence {
  return {
    ok: false,
    error,
    projectPath,
...
export function collectGitEvidence(projectPath: string | null | undefined): GitEvidence {
  ...
  try {
    ...
    const raw = git(
      ['log', `-${COMMIT_LIMIT}`, '--no-merges', '--date=iso-strict', '--pretty=format:%H\u001f%an\u001f%ad\u001f%s'],
      cwd,
    );
    ...
    const evidence = emptyEvidence(cwd, `Could not read commit history: ${(error as Error).message}`);
...
export interface WorkItemGitEvidence {
  ok: boolean;
  error: string | null;
  projectPath: string | null;
  branch: string | null;
 ...
 */
export function matchEvidenceToKeys(evidence: GitEvidence, keys: string[]): WorkItemGitEvidence {
  const needles = keys
    .map((key) => key.trim())
    .filter(Boolean)
  ...
  const commits = needles.length
    ? evidence.commits.filter((commit) => patterns.some((pattern, i) => {
      if (!pattern.test(commit.subject)) return false;
      matched.add(needles[i]);
      return true;
    }))
    : [];

  const branchMatches = Boolean(evidence.branch && patterns.some((p) => p.test(evidence.branch as string)));
