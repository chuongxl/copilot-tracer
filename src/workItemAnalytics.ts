...
import type {
  WorkItemAnalytics,
  WorkItemAnalyticsReport,
  WorkItemGroupAnalytics,
  WorkItemKind,
  WorkItemStatus,
  WorkItemSuccessMeasures,
} from './types.js';

// Phase 7 of the design: productivity analytics over confirmed work items.
//
// Everything here reads recorded facts. Cycle time comes from the status
// history table rather than updated_at, because any edit rewrites updated_at
// and would quietly shorten the number. Where a fact was never recorded the
// result is null, not zero: "no completed items yet" and "completed instantly"
...
function toAnalytics(row: AnalyticsRow): WorkItemAnalytics {
  return {
    id: row.id,
    title: row.title,
...
export function getWorkItemAnalytics(projectId?: string | null): WorkItemAnalytics[] {
  const rows = projectId
    ? db.prepare(`${ITEM_SELECT} WHERE wi.project_id = ? GROUP BY wi.id`).all(projectId)
    : db.prepare(`${ITEM_SELECT} GROUP BY wi.id`).all();
  return (rows as AnalyticsRow[]).map(toAnalytics);
}

function groupBy(
  items: WorkItemAnalytics[],
  pick: (item: WorkItemAnalytics) => string,
): WorkItemGroupAnalytics[] {
  const groups = new Map<string, WorkItemAnalytics[]>();
 ...
 */
export function getSuccessMeasures(projectId?: string | null): WorkItemSuccessMeasures {
  ...
  `).get(...args) as { total: number; with_candidate: number; unlinked: number };
  ...
  `).get(...args) as { accepted: number; decided: number };
...
export function getAnalyticsReport(projectId?: string | null): WorkItemAnalyticsReport {
  const items = getWorkItemAnalytics(projectId);
  const closed = new Set<string>(WORK_ITEM_CLOSED_STATUSES);
