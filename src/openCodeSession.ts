...
export interface OpenCodeUsageDelta {
  input?: number;
  output?: number;
  cached?: number;
  reasoning?: number;
...
const activeBySession = new Map<string, OpenCodeTurnContext>();
/** `${sessionId}:${messageId}` → turn (open or closed). The idempotency key for turns. */
const turnsByKey = new Map<string, OpenCodeTurnContext>();
...
export function detectOpenCodeToolType(name: string): ToolCall['type'] {
  const n = name.toLowerCase();
  if (n.startsWith('mcp__') || n.startsWith('mcp_')) return 'mcp';
  if (n === 'task' || n === 'agent' || n.includes('subagent') || n.includes('delegate')) return 'agent';
...
function persist(ctx: OpenCodeTurnContext, done = false): void {
  recount(ctx.entry);
...
function closeTurn(ctx: OpenCodeTurnContext, opts: { response?: string; error?: string }): void {
  if (!ctx.closed) {
...
export function startTurn(input: {
  ...
  if (existing) {
    ...
    if (!existing.closed) activeBySession.set(input.sessionId, existing);
    persist(existing);
    return existing;
  }
  ...
  if (stale && !stale.closed) closeTurn(stale, {});
...
export function findOpenCodeTurn(sessionId: string, messageId: string | undefined): OpenCodeTurnContext | undefined {
  if (!messageId) return activeBySession.get(sessionId);
...
export function updateTurn(input: {
  ...
  if (input.response) ctx.entry.response = input.response;
  if (input.usage) applyUsage(ctx, input.usage);
  persist(ctx);
  return ctx;
}

function applyUsage(ctx: OpenCodeTurnContext, delta: OpenCodeUsageDelta): void {
 ...
 */
export function finishActiveTurn(input: { sessionId: string; error?: string }): void {
  markSession(input.sessionId);
...
export function endSession(sessionId: string): void {
  ...
  if (ctx && !ctx.closed) closeTurn(ctx, {});
 ...
 */
function toolKey(toolCallId: string | undefined, name: string, input: unknown): string {
 ...
 */
function turnForTool(sessionId: string, messageId: string | undefined, projectId?: string): OpenCodeTurnContext {
  const key = messageId ?? '';
  if (key) {
...
export function resetOpenCodeTracker(): void {
