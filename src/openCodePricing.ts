...
function findRate(model: string): { input: number; output: number } | undefined {
  const key = Object.keys(CREDITS_PER_1K).find(k => model.includes(k));
  return key ? CREDITS_PER_1K[key] : undefined;
}

export function calcOpenCodeCredits(tokens: { input: number; output: number }, model?: string): number {
  if (!model) return 0;
